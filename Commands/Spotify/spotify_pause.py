...
def spotify_pause(user_input, user_name, user_settings, app_instance, log_message):
    try:
        if "pause" in user_input:
            ...
            if device_token:
                ...
                sp.pause_playback(device_token)
                ...
                app_instance.speak(message)
                app_instance.add_to_message_log(message)
                log_message(f"{user_name} attempted to pause playback, but current Spotify device token was not found.")
        else:
            message = "Invalid command for pausing playback."
            app_instance.speak(message)
            app_instance.add_to_message_log(message)
            log_message(f"{user_name} attempted an invalid command for pausing playback.")
